...
_TOOL_ARGUMENT_RULES: dict[str, tuple[frozenset[str], frozenset[str]]] = {
    ...
    "cosmatter_approve_plan": (frozenset({"run_id", "subquestions", "queries", "counter_queries"}), frozenset({"run_id", "subquestions", "queries", "counter_queries", "max_rounds", "max_papers", "query_tracks", "track_candidate_minimums"})),
...
def _tools() -> list[dict[str, object]]:
    return [
        ...
        {"name": "cosmatter_approve_plan", "description": "Persist a human-reviewed bounded FlightPlan. Optional query_tracks bind every primary query index to one of the three independent research tracks and require matching track_candidate_minimums. This is required before an approved-plan search can execute.", "inputSchema": {"type": "object", "additionalProperties": False, "required": ["run_id", "subquestions", "queries", "counter_queries"], "properties": {"run_id": {"type": "string"}, "subquestions": {"type": "array", "items": {"type": "string"}}, "queries": {"type": "array", "items": {"type": "string"}}, "counter_queries": {"type": "array", "items": {"type": "string"}}, "query_tracks": {"type": "array", "items": {"type": "object", "additionalProperties": False, "required": ["query_index", "research_track"], "properties": {"query_index": {"type": "integer", "minimum": 0}, "research_track": {"type": "string", "enum": ["exact_material", "mechanism_analogue", "algorithm"]}}}}, "track_candidate_minimums": {"type": "object", "additionalProperties": False, "required": ["exact_material", "mechanism_analogue", "algorithm"], "properties": {"exact_material": {"type": "integer", "minimum": 1}, "mechanism_analogue": {"type": "integer", "minimum": 1}, "algorithm": {"type": "integer", "minimum": 1}}}, "max_rounds": {"type": "integer", "minimum": 1}, "max_papers": {"type": "integer", "minimum": 1}}}},
