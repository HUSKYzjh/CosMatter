import { normalizeConfig, type CosMatterObservabilityConfig, type NormalizedConfig } from './config.js'
...
const dagSpecs: Record<string, { dependsOn: readonly string[]; descriptors: readonly string[]; dataClassification: string; executionClass: string }> = {
...
export interface WorkflowStage { stage: string; status: string; counts: Record<string, number> }
export interface WorkflowStatus { schema_version: '1.0'; run_id: string; mission_id: string; trust_status: 'loopback_workflow_status_not_scientific_evidence'; next_stage: string | null; stages: WorkflowStage[] }
export interface StageContractStage { stage: string; status: string; completion_requirements: string[]; human_gate: string; expected_outputs: string[]; recovery_route: string; metrics: Record<string, number> }
...
export interface ArtifactManifest { schema_version: 'cosmatter.artifact/v1'; run_id: string; mission_id: string; trust_status: 'allowlisted_artifact_index_not_scientific_evidence'; artifact_count: number; artifacts: ArtifactCard[] }

export class CosMatterObservabilityClient {
  readonly config: NormalizedConfig
  constructor(config: CosMatterObservabilityConfig = {}, private readonly request: typeof fetch = fetch) { this.config = normalizeConfig(config) }

  async workflowStatus(runIdValue: string, signal?: AbortSignal): Promise<WorkflowStatus> {
    if (!runId.test(runIdValue)) throw new Error('run_id is invalid')
    ...
    try {
      ...
      if (length > this.config.maxResponseBytes) throw new Error('CosMatter workflow status response exceeds configured size')
      const text = await response.text()
      if (new TextEncoder().encode(text).byteLength > this.config.maxResponseBytes) throw new Error('CosMatter workflow status response exceeds configured size')
      if (!response.ok) throw new Error(`CosMatter workflow status request failed with HTTP ${response.status}`)
      return validateWorkflowStatus(JSON.parse(text), runIdValue)
    } finally { clearTimeout(timer); signal?.removeEventListener('abort', abort) }
  }

  async artifactManifest(runIdValue: string, signal?: AbortSignal): Promise<ArtifactManifest> {
    if (!runId.test(runIdValue)) throw new Error('run_id is invalid')
    const controller = new AbortController(); const timer = setTimeout(() => controller.abort(), this.config.timeoutMs)
    const abort = () => controller.abort(); signal?.addEventListener('abort', abort, { once: true })
    try {
      ...
      const length = Number(response.headers.get('content-length') ?? 0)
      if (length > this.config.maxResponseBytes) throw new Error('CosMatter artifact manifest response exceeds configured size')
      const text = await response.text()
      if (new TextEncoder().encode(text).byteLength > this.config.maxResponseBytes) throw new Error('CosMatter artifact manifest response exceeds configured size')
      if (!response.ok) throw new Error(`CosMatter artifact manifest request failed with HTTP ${response.status}`)
      return validateArtifactManifest(JSON.parse(text), runIdValue)
    } finally { clearTimeout(timer); signal?.removeEventListener('abort', abort) }
  }

  async stageContract(runIdValue: string, signal?: AbortSignal): Promise<StageContract> {
    if (!runId.test(runIdValue)) throw new Error('run_id is invalid')
    const controller = new AbortController(); const timer = setTimeout(() => controller.abort(), this.config.timeoutMs)
    const abort = () => controller.abort(); signal?.addEventListener('abort', abort, { once: true })
    try {
      ...
      const length = Number(response.headers.get('content-length') ?? 0)
      if (length > this.config.maxResponseBytes) throw new Error('CosMatter stage contract response exceeds configured size')
      const text = await response.text()
      if (new TextEncoder().encode(text).byteLength > this.config.maxResponseBytes) throw new Error('CosMatter stage contract response exceeds configured size')
      if (!response.ok) throw new Error(`CosMatter stage contract request failed with HTTP ${response.status}`)
      return validateStageContract(JSON.parse(text), runIdValue)
    } finally { clearTimeout(timer); signal?.removeEventListener('abort', abort) }
  }

  async operationalTelemetry(runIdValue: string, signal?: AbortSignal): Promise<OperationalTelemetry> {
    if (!runId.test(runIdValue)) throw new Error('run_id is invalid')
    const controller = new AbortController(); const timer = setTimeout(() => controller.abort(), this.config.timeoutMs)
    const abort = () => controller.abort(); signal?.addEventListener('abort', abort, { once: true })
    try {
      ...
      const length = Number(response.headers.get('content-length') ?? 0)
      if (length > this.config.maxResponseBytes) throw new Error('CosMatter operational telemetry response exceeds configured size')
      const text = await response.text()
      if (new TextEncoder().encode(text).byteLength > this.config.maxResponseBytes) throw new Error('CosMatter operational telemetry response exceeds configured size')
      if (!response.ok) throw new Error(`CosMatter operational telemetry request failed with HTTP ${response.status}`)
      return validateOperationalTelemetry(JSON.parse(text), runIdValue)
    } finally { clearTimeout(timer); signal?.removeEventListener('abort', abort) }
  }

  async workflowDag(runIdValue: string, signal?: AbortSignal): Promise<WorkflowDag> {
    if (!runId.test(runIdValue)) throw new Error('run_id is invalid')
    const controller = new AbortController(); const timer = setTimeout(() => controller.abort(), this.config.timeoutMs)
    const abort = () => controller.abort(); signal?.addEventListener('abort', abort, { once: true })
    try {
      ...
      const length = Number(response.headers.get('content-length') ?? 0)
      if (length > this.config.maxResponseBytes) throw new Error('CosMatter workflow DAG response exceeds configured size')
      const text = await response.text()
      if (new TextEncoder().encode(text).byteLength > this.config.maxResponseBytes) throw new Error('CosMatter workflow DAG response exceeds configured size')
      if (!response.ok) throw new Error(`CosMatter workflow DAG request failed with HTTP ${response.status}`)
      return validateWorkflowDag(JSON.parse(text), runIdValue)
    } finally { clearTimeout(timer); signal?.removeEventListener('abort', abort) }
  }
}

export function validateWorkflowStatus(value: unknown, expectedRunId: string): WorkflowStatus {
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error('CosMatter workflow status response is invalid')
  const result = value as Record<string, unknown>
  const expected = new Set(['schema_version', 'run_id', 'mission_id', 'trust_status', 'next_stage', 'stages'])
  if (Object.keys(result).length !== expected.size || Object.keys(result).some(key => !expected.has(key) || sensitiveKey.test(key))) throw new Error('CosMatter workflow status response contains forbidden fields')
  if (result.schema_version !== '1.0' || result.run_id !== expectedRunId || typeof result.mission_id !== 'string' || !result.mission_id || result.trust_status !== 'loopback_workflow_status_not_scientific_evidence' || !(typeof result.next_stage === 'string' || result.next_stage === null) || (typeof result.next_stage === 'string' && !stages.has(result.next_stage)) || !Array.isArray(result.stages) || result.stages.length !== stages.size) throw new Error('CosMatter workflow status response is invalid')
  const seen = new Set<string>()
  for (const stage of result.stages) {
    if (!stage || typeof stage !== 'object' || Array.isArray(stage)) throw new Error('CosMatter workflow stage is invalid')
    const item = stage as Record<string, unknown>
    if (Object.keys(item).length !== 3 || Object.keys(item).some(key => !['stage', 'status', 'counts'].includes(key) || sensitiveKey.test(key)) || typeof item.stage !== 'string' || !stages.has(item.stage) || seen.has(item.stage) || typeof item.status !== 'string' || !states.has(item.status) || !item.counts || typeof item.counts !== 'object' || Array.isArray(item.counts)) throw new Error('CosMatter workflow stage is invalid')
    for (const [key, count] of Object.entries(item.counts as Record<string, unknown>)) {
      if (!key || sensitiveKey.test(key) || typeof count !== 'number' || !Number.isInteger(count) || count < 0 || count > 1_000_000) throw new Error('CosMatter workflow count is invalid')
    }
    seen.add(item.stage)
  }
  return result as unknown as WorkflowStatus
}

export function validateArtifactManifest(value: unknown, expectedRunId: string): ArtifactManifest {
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error('CosMatter artifact manifest response is invalid')
  const result = value as Record<string, unknown>
  const expected = new Set(['schema_version', 'run_id', 'mission_id', 'trust_status', 'artifact_count', 'artifacts'])
  if (Object.keys(result).length !== expected.size || Object.keys(result).some(key => !expected.has(key) || sensitiveKey.test(key))) throw new Error('CosMatter artifact manifest response contains forbidden fields')
  if (result.schema_version !== 'cosmatter.artifact/v1' || result.run_id !== expectedRunId || typeof result.mission_id !== 'string' || !result.mission_id || result.trust_status !== 'allowlisted_artifact_index_not_scientific_evidence' || !Number.isInteger(result.artifact_count) || (result.artifact_count as number) < 0 || !Array.isArray(result.artifacts) || result.artifact_count !== result.artifacts.length) throw new Error('CosMatter artifact manifest response is invalid')
  const seen = new Set<string>()
  for (const item of result.artifacts) {
    if (!item || typeof item !== 'object' || Array.isArray(item)) throw new Error('CosMatter artifact card is invalid')
    const card = item as Record<string, unknown>
    const keys = new Set(['artifact_id', 'title', 'media_type', 'sha256', 'generated_at', 'trust_status', 'download_path'])
    if (Object.keys(card).length !== keys.size || Object.keys(card).some(key => !keys.has(key) || sensitiveKey.test(key)) || typeof card.artifact_id !== 'string' || !artifactIds.has(card.artifact_id) || seen.has(card.artifact_id) || typeof card.title !== 'string' || !card.title || card.title.length > 200 || typeof card.media_type !== 'string' || !/^(application\/json|text\/markdown); charset=utf-8$/.test(card.media_type) || typeof card.sha256 !== 'string' || !sha256.test(card.sha256) || typeof card.generated_at !== 'string' || !card.generated_at || typeof card.trust_status !== 'string' || !artifactTrust.has(card.trust_status) || card.download_path !== `/api/runs/${expectedRunId}/artifacts/${card.artifact_id}`) throw new Error('CosMatter artifact card is invalid')
    seen.add(card.artifact_id)
  }
  return result as unknown as ArtifactManifest
}

export function validateStageContract(value: unknown, expectedRunId: string): StageContract {
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error('CosMatter stage contract response is invalid')
  const result = value as Record<string, unknown>
  const expected = new Set(['schema_version', 'run_id', 'mission_id', 'trust_status', 'next_stage', 'runtime_safety', 'stages'])
  if (Object.keys(result).length !== expected.size || Object.keys(result).some(key => !expected.has(key) || sensitiveKey.test(key))) throw new Error('CosMatter stage contract response contains forbidden fields')
  if (result.schema_version !== 'cosmatter.stage-contract/v1' || result.run_id !== expectedRunId || typeof result.mission_id !== 'string' || !result.mission_id || result.trust_status !== 'loopback_stage_contract_not_scientific_evidence_or_execution_authorization' || !(typeof result.next_stage === 'string' || result.next_stage === null) || (typeof result.next_stage === 'string' && !stages.has(result.next_stage)) || typeof result.runtime_safety !== 'string' || !runtimeSafety.has(result.runtime_safety) || !Array.isArray(result.stages) || result.stages.length !== stages.size) throw new Error('CosMatter stage contract response is invalid')
  const seen = new Set<string>(); let nextStage: string | null = null
  for (const stage of result.stages) {
    if (!stage || typeof stage !== 'object' || Array.isArray(stage)) throw new Error('CosMatter stage contract stage is invalid')
    const item = stage as Record<string, unknown>
    const fields = new Set(['stage', 'status', 'completion_requirements', 'human_gate', 'expected_outputs', 'recovery_route', 'metrics'])
    if (Object.keys(item).length !== fields.size || Object.keys(item).some(key => !fields.has(key) || sensitiveKey.test(key)) || typeof item.stage !== 'string' || !stages.has(item.stage) || seen.has(item.stage) || typeof item.status !== 'string' || !states.has(item.status) || !Array.isArray(item.completion_requirements) || !Array.isArray(item.expected_outputs) || typeof item.human_gate !== 'string' || typeof item.recovery_route !== 'string' || !item.metrics || typeof item.metrics !== 'object' || Array.isArray(item.metrics)) throw new Error('CosMatter stage contract stage is invalid')
    const contract = stageContracts[item.stage]
    if (!contract || JSON.stringify(item.completion_requirements) !== JSON.stringify(contract.requirements) || item.human_gate !== contract.humanGate || JSON.stringify(item.expected_outputs) !== JSON.stringify(contract.outputs) || item.recovery_route !== contract.recoveryRoute) throw new Error('CosMatter stage contract template is invalid')
    for (const [key, count] of Object.entries(item.metrics as Record<string, unknown>)) {
      if (!key || sensitiveKey.test(key) || typeof count !== 'number' || !Number.isInteger(count) || count < 0 || count > 1_000_000) throw new Error('CosMatter stage contract metric is invalid')
    }
    if (nextStage === null && item.status !== 'completed') nextStage = item.stage
    seen.add(item.stage)
  }
  if (result.next_stage !== nextStage) throw new Error('CosMatter stage contract next stage is invalid')
  return result as unknown as StageContract
}
...
export function validateOperationalTelemetry(value: unknown, expectedRunId: string): OperationalTelemetry {
  ...
  for (const raw of result.dispatch_operations) {
    if (!raw || typeof raw !== 'object' || Array.isArray(raw)) throw new Error('CosMatter dispatch operation telemetry is invalid')
    const item = raw as Record<string, unknown>; const fields = new Set(['operation', 'dispatch_count', 'completed_count', 'incomplete_count', 'unknown_outcome_count'])
    if (Object.keys(item).length !== fields.size || Object.keys(item).some(key => !fields.has(key) || sensitiveKey.test(key)) || typeof item.operation !== 'string' || !dispatchOperations.has(item.operation)) throw new Error('CosMatter dispatch operation telemetry is invalid')
  ...
  for (const raw of result.cost_latency) {
    if (!raw || typeof raw !== 'object' || Array.isArray(raw)) throw new Error('CosMatter cost latency telemetry is invalid')
    const item = raw as Record<string, unknown>; const fields = new Set(['provider_id', 'request_count', 'successful_request_count', 'failed_request_count', 'currency', 'total_cost', 'median_latency_seconds', 'p95_latency_seconds'])
    if (Object.keys(item).length !== fields.size || Object.keys(item).some(key => !fields.has(key) || sensitiveKey.test(key)) || typeof item.provider_id !== 'string' || !/^[a-z0-9][a-z0-9_-]{0,79}$/.test(item.provider_id) || costProviders.has(item.provider_id) || typeof item.currency !== 'string' || !currencies.has(item.currency)) throw new Error('CosMatter cost latency telemetry is invalid')
    ...
    if (requestValues.some(value => typeof value !== 'number' || !Number.isInteger(value) || value < 0) || requestValues[0] !== Number(requestValues[1]) + Number(requestValues[2]) || valueKeys.some(key => typeof item[key] !== 'number' || !Number.isFinite(item[key]) || Number(item[key]) < 0) || Number(item.p95_latency_seconds) < Number(item.median_latency_seconds)) throw new Error('CosMatter cost latency telemetry is invalid')
    costProviders.add(item.provider_id)
  }
  return result as unknown as OperationalTelemetry
}

export function validateWorkflowDag(value: unknown, expectedRunId: string): WorkflowDag {
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error('CosMatter workflow DAG response is invalid')
  const result = value as Record<string, unknown>
  ...
  if (Object.keys(result).length !== fields.size || Object.keys(result).some(key => !fields.has(key) || sensitiveKey.test(key))) throw new Error('CosMatter workflow DAG response contains forbidden fields')
  if (result.schema_version !== 'cosmatter.workflow-dag/v1' || result.run_id !== expectedRunId || typeof result.mission_id !== 'string' || !result.mission_id || result.trust_status !== 'loopback_declared_dag_readiness_projection_not_execution_authorization' || result.dag_id !== 'cosmatter_review_gated_linear_workflow' || result.max_concurrency !== 1 || result.scheduler_status !== 'declarative_only_no_execution_authorization' || !runtimeSafety.has(String(result.runtime_safety)) || !Array.isArray(result.eligible_stages) || result.eligible_stages.length > 1 || !Number.isInteger(result.blocked_stage_count) || Number(result.blocked_stage_count) < 0 || Number(result.blocked_stage_count) > stages.size || typeof result.human_review_required !== 'boolean' || !Array.isArray(result.stages) || result.stages.length !== stages.size) throw new Error('CosMatter workflow DAG response is invalid')
  let firstUnfinished: string | null = null; let blocked = 0; let waiting = false
  for (const raw of result.stages) {
    if (!raw || typeof raw !== 'object' || Array.isArray(raw)) throw new Error('CosMatter workflow DAG stage is invalid')
    const item = raw as Record<string, unknown>; const keys = new Set(['stage', 'depends_on', 'status', 'allowed_descriptors', 'data_classification', 'execution_class'])
    if (Object.keys(item).length !== keys.size || Object.keys(item).some(key => !keys.has(key) || sensitiveKey.test(key)) || typeof item.stage !== 'string' || !stages.has(item.stage) || typeof item.status !== 'string' || !states.has(item.status) || !Array.isArray(item.depends_on) || !Array.isArray(item.allowed_descriptors)) throw new Error('CosMatter workflow DAG stage is invalid')
    const spec = dagSpecs[item.stage]
    if (!spec || JSON.stringify(item.depends_on) !== JSON.stringify(spec.dependsOn) || JSON.stringify(item.allowed_descriptors) !== JSON.stringify(spec.descriptors) || item.data_classification !== spec.dataClassification || item.execution_class !== spec.executionClass) throw new Error('CosMatter workflow DAG template is invalid')
    if (firstUnfinished === null && item.status !== 'completed') firstUnfinished = item.stage
    blocked += item.status === 'blocked' ? 1 : 0; waiting ||= item.status === 'waiting_human_review'
  }
  const expectedEligible = firstUnfinished !== null && result.runtime_safety === 'verified' && (result.stages.find(raw => (raw as Record<string, unknown>).stage === firstUnfinished) as Record<string, unknown>).status === 'ready' ? [firstUnfinished] : []
  if (JSON.stringify(result.eligible_stages) !== JSON.stringify(expectedEligible) || result.blocked_stage_count !== blocked || result.human_review_required !== waiting) throw new Error('CosMatter workflow DAG status is invalid')
  return result as unknown as WorkflowDag
}
