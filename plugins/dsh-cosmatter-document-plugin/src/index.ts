...
declare module '@deepseek-ai/cordis' { interface Events { 'cosmatter-document/submitted'(payload: { run_id: string; document_id: string; task_state: string }): void; 'cosmatter-document/polled'(payload: { run_id: string; document_id: string; task_state: string }): void } }

export function apply(ctx: Context, config: CosMatterDocumentConfig = {}): void {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) { const result = await client.submit(String(args.run_id), { authorizations: args.authorizations as string[], document_id: String(args.document_id), source_url: String(args.source_url) }, String(exec.callId), exec.signal); ctx.emit('cosmatter-document/submitted', { run_id: result.run_id, document_id: result.document_id, task_state: result.task_state }); return result as unknown as Record<string, JsonValue> },
  }))
  ctx.tools.register(defineTool({
    ...
    output: { schema: { type: 'object', additionalProperties: true }, render: (_args, value) => [{ type: 'text', text: JSON.stringify(value) }] },
    async execute(args, exec) { const result = await client.poll(String(args.run_id), { authorizations: args.authorizations as string[], document_id: String(args.document_id) }, String(exec.callId), exec.signal); ctx.emit('cosmatter-document/polled', { run_id: result.run_id, document_id: result.document_id, task_state: result.task_state }); return result as unknown as Record<string, JsonValue> },
  }))
