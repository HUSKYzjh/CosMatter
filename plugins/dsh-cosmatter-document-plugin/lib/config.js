export function normalizeConfig(config = {}) {
    ...
    if (parsed.protocol !== 'http:' || parsed.hostname !== '127.0.0.1' || parsed.username || parsed.password || parsed.pathname !== '/' || parsed.search || parsed.hash)
        throw new Error('CosMatter document plugin only permits a bare http://127.0.0.1 endpoint');
    ...
    if (!Number.isInteger(maxResponseBytes) || maxResponseBytes < 4_096 || maxResponseBytes > 131_072)
        throw new Error('maxResponseBytes must be an integer between 4096 and 131072');
