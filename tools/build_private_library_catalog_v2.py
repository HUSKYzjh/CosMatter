#!/usr/bin/env python3
"""Create a deduplicated, non-evidence catalog for private MinerU Markdown."""
...
def title_from_path(value: str) -> str:
    return re.sub(r"\s+", " ", Path(value).stem).strip() or "Untitled local document"
...
def main() -> int:
    ...
    for item in parsed["entries"]:
        ...
        if markdown_sha in seen:
            duplicate_receipts += 1
            continue
        seen.add(markdown_sha)
        documents.append(
            {
                "document_id": f"local-md-{markdown_sha[:24]}",
                "provisional_title": title_from_path(relative),
    ...
    digest = hashlib.sha256(json.dumps(documents, ensure_ascii=False, separators=(",", ":"), sort_keys=True).encode("utf-8")).hexdigest()
    group_counts = {group: sum(item["source_group"] == group for item in documents) for group in sorted({item["source_group"] for item in documents})}
    catalog = {
        "schema_version": "1.0",
        ...
        "catalog_fingerprint": f"sha256:{digest}",
    ...
    template = {
        ...
        "instructions": [
            ...
            "A parsed document is not evidence; EvidenceCard requires a separate human-reviewed Source Map.",
            "For the planned 90-paper BiFeO3 cohort, include only papers matching the frozen question and allowed scope.",
        ],
        "candidates": [
            {
    ...
    print(json.dumps({"document_count": len(documents), "duplicate_parse_receipts_merged": duplicate_receipts, "source_groups": group_counts}, ensure_ascii=False))
    return 0
