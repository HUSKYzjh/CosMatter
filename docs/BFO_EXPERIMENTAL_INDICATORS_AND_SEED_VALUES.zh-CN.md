...
| 室温薄膜磁回线 | `M_s≈150 emu/cm³`、`H_c≈200 Oe`（70 nm）；`M_s≈5 emu/cm³`（400 nm） | 外延 BFO 厚度序列；VSM 面内/面外回线 | 同一论文中厚度变化已产生约 30 倍差异；必须保留衬底扣除、杂相检查、厚度和场方向 | Wang 等，Science 2003，DOI [10.1126/science.1080615](https://doi.org/10.1126/science.1080615) |
| 独立薄膜磁性对照 | `M_s=21.38 emu/cm³`；`M_r=2.06 emu/cm³` | 溶胶凝胶多晶 BFO/Si；室温 VSM，作为 Co 掺杂序列的未掺杂对照 | 是独立作者组的纯 BFO 对照，但不是对 70 nm 外延膜的重复实验 | Bai 等，Nanomaterials 2020，[原文](https://www.mdpi.com/2079-4991/10/9/1798) |
| 纳米颗粒磁矩尺寸序列 | `0.41、0.27、0.13、0.09 μB/Fe`，对应约 `4、15、25、40 nm` | 甘氨酸燃烧法 BFO 粉体；未处理、300 °C/6 h、450 °C/6 h、450 °C/48 h；室温约 ±1.5 T 磁回线 | 颗粒尺寸与热史共同变化，四个点不是四次独立复现；微量杂相与氧缺陷解释仍需 Source Map 核对 | Mazumder 等，APL 2007，DOI [10.1063/1.2768201](https://doi.org/10.1063/1.2768201) |
| 吸收系数 | 500 nm 处：`2.1111×10⁴ cm⁻¹`（200 nm 膜）、`4.8834×10⁴ cm⁻¹`（600 nm 膜）；780 nm 处：`1.07×10⁴ cm⁻¹`（510±23 nm 膜） | 前两项为 ITO/玻璃上的 PLD 膜；后一项为石英上的 RF 溅射多晶膜和 Sellmeier 拟合 | 吸收系数必须连同波长、厚度和提取方法显示，不能只显示“BFO 约为某值” | Singh 等，MRB 2014，DOI [10.1016/j.materresbull.2013.08.050](https://doi.org/10.1016/j.materresbull.2013.08.050)；Gu 等，Optics Express 2009，DOI [10.1364/OE.17.010970](https://doi.org/10.1364/OE.17.010970) |
| 极化可切换光伏 | 向上极化：`V_oc≈+0.17 V`、`J_sc≈−0.61 μA/cm²`；向下极化：`V_oc≈−0.23 V`、`J_sc≈+57 μA/cm²` | Au/BFO/ITO/石英垂直器件；375 nm 连续激光，630 mW/cm² | 符号、极化方向、电极和光强都是数据的一部分；两种状态不能取绝对值后平均 | Zhou 等，Thin Solid Films 2020，DOI [10.1016/j.tsf.2020.137851](https://doi.org/10.1016/j.tsf.2020.137851) |
| 单晶零偏压光电流 | 绿光约 `7.35 μA/cm²`；红光 `2.6 nA/cm²` | 单畴 BFO 单晶、横向 Ag 电极、侧向照明；稳态零偏压光电流 | 这不是标准 one-sun 器件 `J_sc`；正式比较前要复核确切波长、光功率和面积归一化 | Choi 等，Science 2009，DOI [10.1126/science.1168636](https://doi.org/10.1126/science.1168636) |
| 光伏开路电压 | 最高约 50 V | BFO 薄膜，温度、电极间距与电极—畴壁几何受控；异常光伏/体光伏讨论 | 横向异常光伏最大值不能按电压一列直接与亚伏级垂直电容器比较 | Bhatnagar 等，Nature Communications 2013，[原文](https://www.nature.com/articles/ncomms3835) |
...
| 块体与应变薄膜螺旋周期 | 块体 620±20 Å；DyScO₃ 薄膜写入前 78±5 nm、写入后 65±2 nm；SmScO₃ 张应变薄膜 84±3 nm | 块体中子衍射与薄膜 NV 磁测/投影校正采用不同方法；薄膜值还绑定衬底应变和铁电畴状态 | 不能把 `62 nm` 当成所有 BFO 的固定磁性常数；写入前后及投影校正值必须分别存储 | Sosnowska 等，J. Phys. C 1982，DOI [10.1088/0022-3719/15/23/020](https://doi.org/10.1088/0022-3719/15/23/020)；Haykal 等，Nature Communications 2020，[原文](https://www.nature.com/articles/s41467-020-15501-8) |
| PLD 相纯窗口 | 约 580 °C / 0.01 mbar；另一独立装置报告 650 °C / 0.4 mbar | 均为 SrTiO₃(001) 上的 PLD BFO，但腔体、靶材、膜厚与质量判据并不相同 | 两组相差 70 °C 和 40 倍氧压，直接证明温度/氧压不能脱离设备与完整配方作为通用工艺 | Béa 等，APL 2005，DOI [10.1063/1.2009808](https://doi.org/10.1063/1.2009808)；Daumont 等，JMMM 2009，DOI [10.1016/j.jmmm.2009.02.024](https://doi.org/10.1016/j.jmmm.2009.02.024) |
| 沉积速率 | PLD `0.80 Å/s`；MOCVD `4–9 nm/min`；磁控溅射 `0.03 nm/min` | 分别对应 700 °C/3 Pa/5 Hz 的纯 BFO 对照、载气流量变化的 MOCVD、室温沉积后热处理的溅射路线 | 三者跨越约两个数量级；速率只能连同沉积方法、腔体、气氛、温度和标定方法迁移 | Haselmann 等，JPCC 2022，DOI [10.1021/acs.jpcc.2c00674](https://doi.org/10.1021/acs.jpcc.2c00674)；Micard 等，Nanomaterials 2020，DOI [10.3390/nano10040630](https://doi.org/10.3390/nano10040630)；Siadou 等，AMSE 2013，DOI [10.1155/2013/857465](https://doi.org/10.1155/2013/857465) |
| 独立制备批次数 | `3 batches`，共 `17 samples` | 多晶 BFO 陶瓷；三个批次分别在 745、760、780 °C 烧结，并在亚矫顽场循环下做频率分辨原位高能 XRD | 这是“明确报告批次”的正例，但三个批次属于不同烧结温度层，不能解释为同一配方下独立复现 3 次；Wang 2003 与 Zhou 2020 主文仍未找到明确批次数 | Liu 等，Nature Communications 2018，[原文](https://www.nature.com/articles/s41467-018-07363-y) |
| 溶胶凝胶退火温度 | 550 °C 时报告单相；更高温度出现 Bi₂Fe₄O₉ | 溶胶凝胶旋涂薄膜的退火序列，不是 PLD 衬底生长温度 | 作为工艺定义反例；缺少退火时长、前驱体和升降温条件时仍不可复现 | Gao 等，Ceramics International 2013，DOI [10.1016/j.ceramint.2013.04.058](https://doi.org/10.1016/j.ceramint.2013.04.058) |
...
- `configs/bfo_magnetic_optical_process_source_candidate_matrix_v2.json`：继续为磁螺旋周期、直接/间接光学带隙和相纯工艺窗口建立 3 个问题、9 条候选路线。三类查询均真实调用 Sciverse，各返回 5 条带 `doc_id` 的命中，并各完成一次 500 字符受限正文探针；仓库只保存成功状态和调用范围，不保存正文或 provider 文献 ID。
- `configs/bfo_magnetic_pv_process_source_candidate_matrix_v2.json`：为磁回线、吸收系数、光伏响应、沉积速率和独立批次报告缺口建立 5 个问题、16 条候选路线。五类查询均真实调用 Sciverse，各返回 3 条命中并各完成一次 500 字符受限正文探针；另有 Wang 2003、Mazumder 2007 与 Zhou 2020 三条公开 PDF 路线完成私有 MinerU 解析和未审核候选池，不保存返回正文或 provider 文献 ID。
...
- `examples/frozen/bfo_magnetic_optical_process_literature_observation_candidates_v2.json`：新增 15 条机器可读候选观测，分别保存 5 条螺旋周期、5 条直接/间接带隙和 5 条 PLD/溶胶凝胶工艺值。写入前/后螺旋周期、直接/间接拟合、PLD 生长温度/氧压和溶胶凝胶退火温度均使用独立记录；全部保持 `literature_mentioned + unreviewed + source_map_status=none`。
- `examples/frozen/bfo_magnetic_pv_process_literature_observation_candidates_v2.json`：新增 25 条候选观测，保存厚度/形貌/电场绑定的磁性值、4–40 nm 粒径与热史绑定的四点磁矩序列、波长和厚度绑定的吸收系数、带符号的 `V_oc/J_sc` 状态、三种沉积路线的速率，以及一条明确报告 `3 batches / 17 samples` 的条件分层批次记录。微安与纳安原始单位不提前换算；不同烧结温度的批次不冒充同条件独立复现。
- `src/cosmatter/material_indicator_registry.py`：无依赖校验器，拒绝缺字段、越级成熟度、伪造 Source Map 绑定、非法单位和不完整的范围/误差语义。
- `src/cosmatter/material_indicator_triage.py` 与 `tools/build_private_bfo_indicator_shortlist.py`：可对仓库外且哈希核验通过的整篇 MinerU Markdown 做确定性排序，不再受通用 48 段抽样限制；排序综合指标术语、数值/单位、测量条件、方法和限制语句，并对参考文献、引用型比较与非目标性质降权。命令现支持四份版本化 BFO 来源矩阵，并识别 `M_s/M_r/H_c`、`μB/Fe`、吸收系数、`V_oc/J_sc`、沉积速率和批次数的常见正文/公式写法。每篇最多 2 段、全批最多 12 段；结果保留原文及双重哈希，只是私有导航候选，不是 Source Map 或证据。
...
2. **已完成候选层**：四个核心 P0 问题和 12 个 v2 扩展问题均具备“主路线 + 独立实验组 + 条件/定义反例”；`M_s/M_r/H_c`、每 Fe 磁矩、吸收系数、`V_oc/J_sc` 和沉积速率已有条件绑定候选值。独立批次数已有一条 `3 batches / 17 samples` 的明确报告，但它按三个烧结温度分层，不能作为同条件复现；薄膜路线的同条件独立批次数仍是覆盖缺口。下一步是人工建立 Source Map 并逐条核对图表、单位和完整条件。
