...
- `configs/bfo_magnetic_pv_process_source_candidate_matrix_v2.json`：为磁回线、吸收系数、光伏响应、沉积速率和独立批次报告缺口建立 5 个问题、15 条候选路线。五类查询均真实调用 Sciverse，各返回 3 条命中并各完成一次 500 字符受限正文探针；只登记成功状态和范围，不保存返回正文或 provider 文献 ID。
...
- `examples/frozen/bfo_magnetic_pv_process_literature_observation_candidates_v2.json`：新增 21 条候选观测，保存厚度/形貌/电场绑定的磁性值、波长和厚度绑定的吸收系数、带符号的 `V_oc/J_sc` 状态、三种沉积路线的速率，以及一条明确报告 `3 batches / 17 samples` 的条件分层批次记录。微安与纳安原始单位不提前换算；不同烧结温度的批次不冒充同条件独立复现。
