...
同一门禁也已用于三条 BFO 受控试点。第四版跨运行私有聚合登记包含 3 条限定主张、10 条支撑记录与 10 个自动 Source Map；当前链接审计为零错误。相变问题已有实验、模拟和一条冲突性相指认路线，登记表用 `contradicts` 保留约 950°C 立方/金属转变争议而不自动裁决；薄膜应变问题已有第一性原理、厚度/温度演化实验和 Raman 变温实验三条路线，其中厚度失稳作为单一应变阈值叙事的条件边界；缺陷稳定性问题新增了独立作者群的薄膜局域电导实验，但该实验以氧含量/电流变化为氧空位解释的代理，研究终点也不是畴壁稳定性，因此记录为 `boundary_counterexample` 而非独立复现。三条主张都保持在 `literature_mentioned`，且独立性、数值、条件和可复现性均未标记为人工核验。
