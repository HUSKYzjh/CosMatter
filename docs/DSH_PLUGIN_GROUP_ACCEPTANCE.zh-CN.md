...
| 聚合运行遥测 | `tests/test_operational_telemetry.py`、`tests/test_validation_rejections.py`、`LocalMissionApi.operational_telemetry` 与 `cosmatter_operational_telemetry` 使用 `cosmatter.operational-telemetry/v2`：仅按 receipt/dispatch 状态和固定的本地参数拒绝类别汇总次数，不保留参数值、路径、问题、文献标识或正文；成本/延迟只在已人工复核披露存在时转发，任何聚合都不能被当作账单、provider 性能或自动重试指令。 |
