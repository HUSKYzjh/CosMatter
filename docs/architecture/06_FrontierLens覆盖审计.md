...
1. **浏览器级交互验证**：已有 HTML/JS 静态检查及单元测试，但没有本地 HTTP
   服务上的键盘操作、错误 JSON、空工件和截图基线测试。
2. **材料特异关系模型**：已将已批准证据卡中的标量条件字段投影为实验/计算
   设置节点，并使用“报告条件，非因果”边连接。单位后缀随字段名保留；尚未
   建立可换算单位本体或完整材料知识图谱。
3. **多源关系交叉核验**：OpenAlex 与 Crossref 都可由已批准、带 DOI 的证据卡触发；
   两类边分别投影到 CLI/UI，Crossref 至多保留 12 条参考 DOI，并区分“字段未登记”
   与“零引用”。尚未建立两源目标标识的可审计同一性映射，也不得因单一元数据源
   缺失而推断不存在引用。
4. **长期运行任务**：现有 CLI 是短同步调用；若引入队列或后台 worker，必须把
