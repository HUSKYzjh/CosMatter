...
export function PrivateMaterialFactReview(props: {
  ...
  return <Show when={props.segments.length}><section class="private-material-fact-review">
    ...
    <div class="source-map-controls"><ActionButton class="quiet-action" size="sm" variant="quiet" disabled={facts().length >= 48} onClick={() => setFacts((current) => [...current, emptyFact(props.segments[0]?.segment_id, current.length + 1)])}>{tr("添加事实", "Add fact")}</ActionButton><label class="source-map-confirm"><input type="checkbox" checked={confirmed()} onChange={(event) => setConfirmed(event.currentTarget.checked)} />{tr("我已人工核对每一事实均由所选来源片段支持，并同意将其登记为本地、非结论性的结构化材料事实。", "I verified each fact against its selected source excerpt and consent to recording it locally as a non-conclusive structured material fact.")}</label><ActionButton class="primary-action" variant="primary" busy={busy()} busyLabel={tr("登记中…", "Recording…")} disabled={!valid()} onClick={() => void submit()}>{tr("登记人工材料事实", "Record human material facts")}</ActionButton></div>
