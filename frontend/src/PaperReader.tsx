...
export function PaperReader(props: { bundle: ImportedBundle; session: ResearchSession; pdfTask: PdfTaskStatus | null; screeningAllowsSourceReview: boolean; markdownUrl: string | null; onRecordSourceMap?: (segments: PrivateSourceMapSegment[]) => Promise<SourceMapRecordResult>; onLoadSourceMap?: () => Promise<SourceMapRecordResult>; onRecordMaterialFacts?: (facts: HumanMaterialFactInput[]) => Promise<void>; onRecordEvidence?: (input: HumanEvidenceReviewInput) => Promise<EvidenceReviewResult>; readOnlyPreview?: boolean; onExitPreview?: () => void; onNavigate: (view: View) => void; onSelectEvidence: (evidence: EvidenceCard) => boolean }) {
  ...
  return <main class="discovery-stage reader-stage">
    ...
    <Show when={props.session.selectedNode} fallback={<section class="reader-empty"><h2>{tr("尚未选择待核对论文")}</h2><p>{tr("先在文献星图选择一篇候选论文；独立 PDF 可用于私有 Markdown 与书目导航，但未与人工筛选候选关联时，不能登记 Source Map 或 EvidenceCard。")}</p><ActionButton variant="primary" onClick={() => props.onNavigate("graph")}>{tr("返回文献星图")}</ActionButton></section>}>
      <section class="reader-selection"><div><small>{tr("当前论文")}</small><strong>{props.session.selectedNode!.label}</strong><span>{props.session.selectedNode!.source ?? tr("本地工件")}</span></div><ActionButton size="sm" variant="secondary" onClick={() => props.onNavigate("graph")}>{tr("更换论文")}</ActionButton></section>
      <Show when={props.bundle.materialFacts && !currentMaterialFacts()}><section class="reader-intake-note"><small>{tr("其他文献的事实已隐藏", "FACTS FOR ANOTHER PAPER HIDDEN")}</small><p>{tr("本任务存在与当前论文不匹配的已登记材料事实；为避免跨文献混用，本页不会显示它们。请在星图选择对应论文后再核对。", "This mission has recorded material facts for a paper other than the current selection. They are hidden here to prevent cross-paper mixing; select the matching paper in the map to review them.")}</p><ActionButton size="sm" variant="secondary" onClick={() => props.onNavigate("graph")}>{tr("返回文献星图", "Return to literature map")}</ActionButton></section></Show>
      ...
      <Show when={sourceAction() === "await_private_parsing"}>
        <section class="reader-intake-note"><small>{tr("私有解析进行中", "PRIVATE PARSING IN PROGRESS")}</small><p>{tr("当前论文的授权 PDF 已绑定，但 MinerU 私有解析尚未完成。解析完成前不会显示 Markdown、登记来源定位或接受 EvidenceCard。", "An authorised PDF is bound to this paper, but private MinerU parsing is not finished. Markdown, source locations, and EvidenceCard acceptance stay unavailable until it completes.")}</p><ActionButton size="sm" variant="secondary" onClick={() => props.onNavigate("workflow")}>{tr("返回舰桥查看解析状态", "Return to bridge for parsing status")}</ActionButton></section>
      </Show>
      <Show when={sourceAction() === "retry_authorized_pdf"}>
        <section class="reader-intake-note"><small>{tr("私有解析失败", "PRIVATE PARSING FAILED")}</small><p>{tr("当前论文此前绑定的授权 PDF 未能完成私有解析。该失败不构成来源定位或证据；请返回星图重新选择你有权处理的 PDF。", "The authorised PDF previously bound to this paper did not finish private parsing. This failure is not a source location or evidence; return to the map and choose a PDF you are allowed to process again.")}</p><ActionButton size="sm" variant="secondary" onClick={() => props.onNavigate("graph")}>{tr("返回星图重新选择授权 PDF", "Return to map and choose authorised PDF again")}</ActionButton></section>
      </Show>      <Show when={intake().attachedDocumentId && !intake().matchingPrivatePdf}>
        <section class="reader-intake-note"><small>{tr("来源定位尚未对齐", "SOURCE INTAKE NOT ALIGNED")}</small><p>{tr(`当前私有 PDF 只关联候选 ${intake().attachedDocumentId}，而非当前论文 ${intake().selectedDocumentId ?? "未知"}。请返回星图选择该候选，或为当前论文完成筛选后再附加授权 PDF。`, `The current private PDF is attached only to candidate ${intake().attachedDocumentId}, not the selected paper ${intake().selectedDocumentId ?? "unknown"}. Select that candidate in the map, or screen the current paper before attaching an authorised PDF.`)}</p><ActionButton size="sm" variant="secondary" onClick={() => props.onNavigate("graph")}>{tr("返回文献星图对齐论文", "Return to map and align paper")}</ActionButton></section>
      </Show>
      <Show when={!intake().matchingPrivatePdf && !intake().attachedDocumentId && !intake().hasLinkedEvidence}>
        <section class="reader-intake-note"><small>{tr("尚缺授权全文工件", "AUTHORISED FULL TEXT PENDING")}</small><p>{sourceAction() === "choose_authorized_pdf" ? tr("当前论文已被人工纳入全文核对，但尚未绑定对应的授权 PDF。请返回星图选择你有权处理的 PDF；未完成来源定位前不会建立 EvidenceCard。", "This paper is already human-included for full-text review but has no matching authorised PDF. Return to the map and choose a PDF you are allowed to process; no EvidenceCard is created before source location.") : tr("当前论文还没有与人工筛选结果匹配的私有 PDF，也没有已导入的溯源证据。请先在星图完成候选筛选，再选择有权处理的 PDF。", "The selected paper has neither a private PDF matched to a human screening decision nor imported provenance-linked evidence. Complete candidate screening in the map, then choose a PDF you are authorised to process.")}</p><ActionButton size="sm" variant="secondary" onClick={() => props.onNavigate("graph")}>{sourceAction() === "choose_authorized_pdf" ? tr("返回星图选择授权 PDF", "Return to map and choose authorised PDF") : tr("返回星图进行候选筛选", "Return to map for candidate screening")}</ActionButton></section>
      </Show>
