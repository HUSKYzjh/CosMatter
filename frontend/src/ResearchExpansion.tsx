...
export function ResearchExpansion(props: { bundle: ImportedBundle; session: ResearchSession; onNavigate: (view: View) => void; onFocusEvidence?: (evidence: EvidenceCard) => void; onOpenTaskControl?: () => void; onBuildConditionMatrix?: () => Promise<void>; onBuildGapCandidates?: () => Promise<void>; readOnlyPreview?: boolean; onExitPreview?: () => void }) {
  ...
  return <main class="discovery-stage expansion-stage">
    ...
    <section class="simulation-campaign-panel" aria-label={tr("计算活动计划", "Simulation campaign plan")}>
      <header><div><small>{tr("COMPUTATIONAL CAMPAIGN / PLAN ONLY", "COMPUTATIONAL CAMPAIGN / PLAN ONLY")}</small><h2>{tr("计算活动保持在批准的计划边界内", "Computational activity remains inside the approved planning boundary")}</h2></div><span>{props.bundle.simulationCampaignStatus.replaceAll("_", " ")}</span></header>
      <Show when={props.bundle.simulationCampaign} fallback={<p>{props.bundle.simulationCampaignStatus === "rejected" ? tr("导入的计算活动记录未通过计划边界校验，故未显示；浏览器不会尝试恢复、执行或调用外部计算。", "The imported campaign did not pass planning-boundary validation and is hidden. The browser will not recover, execute, or call external computation.") : tr("本任务尚未导入已批准的计算活动计划。DFT、MD、势函数评测与训练均保持禁用。", "No approved simulation campaign is imported for this task. DFT, MD, potential benchmarking, and training remain disabled.")}</p>}>
        {(campaign) => <><dl><div><dt>{tr("活动类型", "campaign kind")}</dt><dd>{campaign().simulationKind}</dd></div><div><dt>{tr("已绑定证据", "bound evidence")}</dt><dd>{campaign().evidenceCount}</dd></div><div><dt>{tr("输入清单", "input manifest")}</dt><dd>{campaign().inputCount}</dd></div><div><dt>{tr("执行预算", "execution budget")}</dt><dd>{campaign().budget.maxJobs}/{campaign().budget.maxGpuJobs}/{campaign().budget.maxDftJobs}</dd></div></dl><ol class="simulation-campaign-chain"><li>{tr("证据", "Evidence")} · {campaign().chain.evidence}</li><li>{tr("假设", "Hypothesis")} · {campaign().chain.hypothesis}</li><li>{tr("协议", "Protocol")} · {campaign().chain.protocol}</li><li>{tr("执行", "Execution")} · {campaign().chain.execution}</li></ol><p>{campaign().continuationReason}</p></>}
      </Show>
    </section>
    <section class="simulation-campaign-panel" aria-label={tr("外部计算结果审核", "External simulation result review")}>
      <header><div><small>{tr("EXTERNAL RESULT / READ ONLY", "EXTERNAL RESULT / READ ONLY")}</small><h2>{tr("外部结果必须经人工审阅，且尚未进入证据卡", "External results remain human-reviewed and outside EvidenceCards")}</h2></div><span>{props.bundle.simulationEvidenceStatus.replaceAll("_", " ")}</span></header>
      <Show when={props.bundle.simulationEvidence} fallback={<p>{props.bundle.simulationEvidenceStatus === "rejected" ? tr("结果回执或人工审核未通过链路校验，未显示。", "The result receipt or human review failed provenance validation and is hidden.") : tr("尚未导入并人工审核外部计算的聚合结果。", "No aggregate external result has been imported and human-reviewed.")}</p>}>
        {(evidence) => <dl><div><dt>{tr("结果类别", "result kind")}</dt><dd>{evidence().resultKind}</dd></div><div><dt>{tr("收敛状态", "convergence")}</dt><dd>{evidence().convergenceStatus}</dd></div><div><dt>{tr("与假设关系", "relation")}</dt><dd>{evidence().relationToHypothesis}</dd></div></dl>}
      </Show>
      <Show when={props.bundle.simulationEvidence}>{(evidence) => <p>{evidence().applicabilityBoundary}<br />{evidence().uncertainty}<br />{evidence().resultInterpretationBoundary}</p>}</Show>
    </section>
    ...
    <section class="evidence-session expansion-gate"><div><small>{tr("当前证据门禁")}</small><strong>{gate().ready ? tr("已选择带来源定位的已接受 EvidenceCard") : tr("研究拓展尚未解锁")}</strong><span>{gate().ready ? `${evidence()!.evidenceId} · ${evidence()!.provenance.documentId} · ${evidence()!.provenance.locator}` : reason()}</span></div><ActionButton size="sm" variant="secondary" onClick={() => props.onNavigate(gate().reason === "paper" ? "graph" : "reader")}>{gate().reason === "paper" ? tr("返回文献星图") : tr("返回证据核对")}</ActionButton></section>
    ...
    <Show when={gate().ready} fallback={<section class="gap-empty"><h2>{tr("尚不能把候选作为后续研究方向")}</h2><p>{tr("请补齐当前会话的论文选择、已接受 EvidenceCard 和来源定位；本页不会以全局计数替代当前审计链路。")}</p></section>}>
      ...
      <section class="comparison-readiness" aria-live="polite">
        ...
        <section class="extension-next-action" aria-live="polite">
          ...
          <Show when={nextAction() === "comparison-evidence"}><ActionButton variant="primary" onClick={() => props.onNavigate("graph")}>{nextActionCopy().button}</ActionButton></Show>
          <Show when={nextAction() === "provenance-audit"}><ActionButton variant="primary" onClick={() => props.onNavigate("reader")}>{nextActionCopy().button}</ActionButton></Show>
          <Show when={nextAction() === "counterevidence"}><ActionButton variant="primary" onClick={() => { if (props.onOpenTaskControl) props.onOpenTaskControl(); else props.onNavigate("discover"); }}>{nextActionCopy().button}</ActionButton></Show>
          <Show when={nextAction() === "condition-matrix"}><Show when={props.onBuildConditionMatrix} fallback={<ActionButton variant="primary" onClick={() => props.onNavigate("workflow")}>{nextActionCopy().button}</ActionButton>}><ActionButton variant="primary" busy={actionBusy()} busyLabel={tr("生成中…", "Generating…")} onClick={() => void runAction(props.onBuildConditionMatrix)}>{nextActionCopy().button}</ActionButton></Show></Show>
          <Show when={nextAction() === "gap-candidates"}><Show when={props.onBuildGapCandidates} fallback={<ActionButton variant="primary" onClick={() => props.onNavigate("workflow")}>{nextActionCopy().button}</ActionButton>}><ActionButton variant="primary" busy={actionBusy()} busyLabel={tr("生成中…", "Generating…")} onClick={() => void runAction(props.onBuildGapCandidates)}>{nextActionCopy().button}</ActionButton></Show></Show>
        </section>
