...
const task = (state: string): PdfTaskStatus => ({
...
describe("private PDF polling", () => {
  it("polls only a local run with an in-progress task", () => {
    expect(shouldPollPdfTask("run-1", task("submitted"))).toBe(true);
    expect(shouldPollPdfTask(null, task("submitted"))).toBe(false);
    expect(shouldPollPdfTask("run-1", null)).toBe(false);
  });

  it.each(["done", "failed", "cancelled"])("stops polling terminal state %s", (state) => {
