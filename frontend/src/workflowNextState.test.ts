...
const task = (overrides: Partial<PdfTaskStatus> = {}): PdfTaskStatus => ({
  ...
  file_name: "paper.pdf", state: "submitted", doi: null, doi_status: "pending", markdown_ready: false, source_map_review_status: "absent", source_map_segment_count: 0,
...
describe("workflow next action", () => {
  it("routes a screened, parsed PDF to selecting its attached paper first", () => {
    expect(workflowNextState(3, task({ candidate_document_id: "paper-1", markdown_ready: true, audit_state: "done" }))).toBe("select-attached-paper");
  });

  it("continues directly to reader work only when the attached paper is already selected in this session", () => {
    const parsed = task({ candidate_document_id: "paper-1", markdown_ready: true, audit_state: "done" });
  ...
  it("keeps the attached-paper selection step even after source locations are recorded", () => {
    expect(workflowNextState(3, task({ candidate_document_id: "paper-1", markdown_ready: true, audit_state: "done", source_map_review_status: "recorded", source_map_segment_count: 2 }))).toBe("select-attached-paper");
  });
  it("promotes citation expansion only for an unlinked PDF with a confirmed DOI", () => {
    expect(workflowNextState(0, task({ markdown_ready: true, audit_state: "done", doi_status: "resolved", doi: "10.1000/example" }))).toBe("citation-map");
    expect(workflowNextState(3, task({ candidate_document_id: "paper-1", markdown_ready: true, audit_state: "done", doi_status: "resolved", doi: "10.1000/example" }))).toBe("select-attached-paper");
  });

  it("keeps an unlinked parsed PDF on the private Markdown navigation branch until its DOI is confirmed", () => {
    expect(workflowNextState(0, task({ markdown_ready: true, audit_state: "done" }))).toBe("standalone-markdown");
  });
  ...
  it("never projects a private PDF task onto a different selected paper session", () => {
    ...
    expect(workflowNextState(3, task({ candidate_document_id: "paper-1", markdown_ready: true, audit_state: "done" }), "paper-2")).toBe("align-pdf-context");
  });
