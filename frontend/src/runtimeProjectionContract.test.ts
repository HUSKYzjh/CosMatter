...
describe("trusted runtime projections", () => {
  ...
  it("fails closed for a malformed contract, DAG, or telemetry projection", () => {
    ...
    const rawRejection = telemetry(); rawRejection.validation_rejections = [{ command: "sciverse_read_context", reason_code: "limit_above_maximum", rejection_count: 0 }];
    expect(trustedRuntimeProjections("run_1", contract(), dag(), rawRejection)).toBe(false);
    const acceptedRejection = telemetry(); acceptedRejection.validation_rejections = [{ command: "sciverse_read_context", reason_code: "limit_above_maximum", rejection_count: 2 }];
    expect(trustedRuntimeProjections("run_1", contract(), dag(), acceptedRejection)).toBe(true);
  });
