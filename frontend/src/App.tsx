...
import { CorpusRelevanceReviewDesk } from "./CorpusRelevanceReviewDesk";
...
import { getDshProfileStatus, getFacilityContractCatalogue, getHarnessPluginCatalogue, getOperationalTelemetry, getReminderBoard, getStageContract, getWorkflowDag, isDshProfileStatus, isFacilityContractCatalogue, isHarnessPluginCatalogue, isLocalApiStatus, isReminderBoard, type DshProfileHealth, type DshProfileStatus, type FacilityCatalogueHealth, type FacilityContractManifest, type HarnessCatalogueHealth, type HarnessPluginDescriptor, type OperationalTelemetry, type ReminderBoard, type StageContract, type WorkflowDag } from "./localApi";
...
export function App() {
  ...
  const [dshProfile, setDshProfile] = createSignal<DshProfileStatus | null>(null);
  const [dshProfileHealth, setDshProfileHealth] = createSignal<DshProfileHealth>("disabled");
  ...
  const validationRejectionCount = createMemo(() => operationalTelemetry()?.validation_rejections.reduce((total, item) => total + item.rejection_count, 0) ?? 0);
  ...
  const hasNavigableEvaluationAudit = () => {
    const readiness = bundle().auditSummary.submissionReadiness;
    return Object.values(bundle().auditSummary.evaluation).some(Boolean)
      || Boolean(readiness.frozenCorpus || readiness.humanAnnotation || readiness.bibliographicSource);
  };
  ...
  function navigate(next: View) {
    ...
    if (next === "horizon" && !gate().ready && !hasNavigableEvaluationAudit()) {
    ...
    if (stage?.state === "blocked" && !(next === "horizon" && hasNavigableEvaluationAudit())) {
  ...
  async function refreshDshProfile() {
    const requestEpoch = ++dshProfileRequestEpoch;
    if (launchPreview() || !localApiEnabled()) {
      setDshProfile(null);
      setDshProfileHealth("disabled");
      return;
    }
    setDshProfileHealth("loading");
    try {
      const status = await getDshProfileStatus();
      if (requestEpoch !== dshProfileRequestEpoch) return;
      if (!isDshProfileStatus(status)) throw new Error("invalid DSH profile snapshot");
      setDshProfile(status);
      setDshProfileHealth("ready");
    } catch {
      if (requestEpoch !== dshProfileRequestEpoch) return;
      setDshProfile(null);
      setDshProfileHealth("unavailable");
    }
  }
  createEffect(() => {
    if (launchPreview()) {
      dshProfileRequestEpoch += 1;
      setDshProfile(null);
      setDshProfileHealth("disabled");
      return;
    }
    void refreshDshProfile();
  });
  createEffect(() => {
    const runId = liveRunId();
  ...
  return <div class={`workbench theme-${theme()} view-${view()}`}>
    ...
    <aside class="research-rail" aria-label={text("研究任务导航", "Research task navigation")}>
      ...
      <section class={`session-handoff state-${sessionHandoff().state}`} aria-label={text("当前审核锚点", "Current review anchor")}>
        ...
        <ActionButton size="sm" onClick={() => navigate(sessionHandoff().destination)}>{sessionHandoffAction(sessionHandoff().state)}</ActionButton>
      </section>
      ...
      <Show when={liveRunId() && localApiEnabled()}>
        <section class="rail-runtime" aria-live="polite" aria-label={text("本地运行状态", "Local runtime status")}>
          ...
          <ActionButton size="sm" busy={runtimeProjectionRefreshBusy()} busyLabel={text("正在刷新本机投影…", "Refreshing local projection…")} onClick={() => void refreshRuntimeProjectionsManually()}>{text("立即刷新本机运行态", "Refresh local runtime now")}</ActionButton>
          ...
          <Show when={runtimeStageRecovery()}>{(recovery) => <section class="runtime-stage-recovery" aria-label={text("阶段恢复导航", "Stage recovery navigation")}><small>{text("阶段恢复导航 / 不执行", "STAGE RECOVERY NAVIGATION / NO EXECUTION")}</small><p>{text("此入口只打开当前阶段对应的本地审核界面；仍须在该界面逐项确认人工门和授权。", "This opens only the local review surface for the current stage; its human gates and authorisations still require item-by-item confirmation there.")}</p><ActionButton size="sm" onClick={openRuntimeStageRecovery}>{stageRecoveryLabel(recovery().target)}</ActionButton></section>}</Show>
          ...
          <Show when={operationalTelemetry()}>{(telemetry) => <p>{text(`已记录的本地 provider 调用：${telemetryRequestCount()}；参数拒绝：${validationRejectionCount()}；成本/时延披露：${telemetry().cost_latency_status}。`, `Recorded local provider calls: ${telemetryRequestCount()}; parameter rejections: ${validationRejectionCount()}; cost/latency disclosure: ${telemetry().cost_latency_status}.`)}</p>}</Show>
        </section>
        <section class="rail-execution" aria-label={text("本地任务控制", "Local task control")}><small>{text("受控执行", "CONTROLLED EXECUTION")}</small><p>{text("停止后不会再接收该任务的迟到结果；已登记工件保留供回看。", "After stopping, late results for this task are ignored; registered artifacts remain reviewable.")}</p><ActionButton size="sm" variant="danger" onClick={stopCurrentRun}>{text("停止当前本地任务", "Stop current local task")}</ActionButton></section>
       </Show>
      </details>
    ...
    <Suspense fallback={<main class="route-loading" aria-live="polite"><div class="route-loading-beacon" aria-hidden="true"><i /><i /><i /></div><small>{text("正在切换工作区", "SWITCHING WORKSPACE")}</small><h1>{text("正在载入已选择的本地工作区", "Loading the selected local workspace")}</h1><p>{text("不会读取私有文件、调用提供方或改变任务。", "No private file is read, provider is called, or task is changed.")}</p><div class="route-loading-radar" aria-hidden="true"><i /><i /><i /></div></main>}>
      ...
      <main class="discovery-stage mission-stage"><FleetDecoration kind="discover" state={fleetVisualState(bundle(), "discover")} />
        ...
        <section class="mission-definition-form" aria-label={text("任务草稿", "Mission draft")}>
          ...
          <footer class="mission-definition-actions"><span classList={{ "draft-changed": !missionDraftReady() || missionDraftChanged(), "draft-synced": missionDraftReady() && !missionDraftChanged() }}>{launchPreview() ? text("只读预览：任务确认已锁定", "Read-only preview: mission confirmation is locked") : !missionDraftReady() ? text(`还需填写：${missionDraftMissing().join("、")}`, `Still required: ${missionDraftMissing().join(", ")}`) : missionDraftChanged() ? text("存在尚未确认的任务变更", "Unconfirmed task changes") : text("草稿与当前任务已同步", "Draft matches the current mission")}</span><ActionButton variant="primary" size="lg" disabled={launchPreview() || !missionDraftReady()} onClick={enterBridge}>{launchPreview() ? text("只读预览：不能确认任务", "Read-only preview: confirmation unavailable") : text("确认任务并进入受控编排", "Confirm mission and enter orchestration")}</ActionButton></footer>
        </section>
        <Show when={!launchPreview()}><QuestionSetReviewDesk locale={language()} /><CorpusRelevanceReviewDesk locale={language()} /></Show>
