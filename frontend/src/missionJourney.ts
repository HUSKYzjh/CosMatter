...
export function missionJourney(
  ...
  const extended = comparison.ready && counterevidence.ready && bundle.researchGapCandidates.some((candidate) => hasAuditableGapEvidenceBasis(candidate, bundle) && hasExecutedGapCounterevidenceBoundary(candidate, counterevidence));
