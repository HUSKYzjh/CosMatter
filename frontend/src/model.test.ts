...
describe("readBundle", () => {
  it("reads the complete safe UI projection", () => {
    ...
    expect(bundle.importDiagnostics).toEqual({ declaredAcceptedEvidenceCount: 1, withheldAcceptedEvidenceCount: 0 });
  ...
  it("retains only a valid bounded generation timestamp", () => {
    const valid = readBundle({ schema_version: "1.0", generated_at: "2026-08-03T00:00:00+00:00", mission: { mission_id: "m", question: "q", material: "BiFeO3", property_name: "phase", scope: "films" } });
    const invalid = readBundle({ schema_version: "1.0", generated_at: "not-a-timestamp", mission: { mission_id: "m", question: "q", material: "BiFeO3", property_name: "phase", scope: "films" } });
  ...
  it("rejects duplicate or malformed accepted EvidenceCards instead of guessing a safe copy", () => {
    ...
    expect(bundle.importDiagnostics).toEqual({ declaredAcceptedEvidenceCount: 4, withheldAcceptedEvidenceCount: 4 });
  });
});
