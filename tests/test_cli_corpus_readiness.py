...
class CliCorpusReadinessTests(unittest.TestCase):
    ...
    def test_prepares_all_blank_real_evaluation_artifacts_from_one_frozen_manifest(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            candidates_path = run / "retrieval_candidates.json"
            candidates = json.loads(candidates_path.read_text(encoding="utf-8"))
            seeded_candidate_bytes = candidates_path.read_bytes()
            unseeded_output = io.StringIO()
            with patch("cosmatter.cli._runs_dir", return_value=runs), contextlib.redirect_stdout(unseeded_output):
                unseeded_status = main(["prepare-real-evaluation", "--run-id", "evaluation_pack", "--expected-count", "1"])
            unseeded_result = json.loads(unseeded_output.getvalue())
            candidate_bytes_after_unseeded_preparation = candidates_path.read_bytes()
        self.assertEqual(status, 0, output.getvalue())
        self.assertEqual(result["evaluation_gate"], "ready_for_private_human_annotation")
        ...
        self.assertIsNone(unseeded_result["candidates_path"])
        self.assertEqual(candidate_bytes_after_unseeded_preparation, seeded_candidate_bytes)
