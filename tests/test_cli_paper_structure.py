...
class CliPaperStructureTests(unittest.TestCase):
    def test_records_only_structure_tied_to_reviewed_source_map(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            (run / "source_map.json").write_text(json.dumps({"schema_version":"1.0","mission_id":"mission_1","trust_status":"human_reviewed_parser_selection","document_id":"doc_1","provider":"mineru","task_id_sha256":"a"*64,"segments":[{"segment_id":"p1","locator":"p.1","kind":"paragraph","quote":"quoted","quote_sha256":""}]}), encoding="utf-8")
            source = json.loads((run / "source_map.json").read_text(encoding="utf-8")); import hashlib; source["segments"][0]["quote_sha256"] = hashlib.sha256(b"quoted").hexdigest(); (run / "source_map.json").write_text(json.dumps(source), encoding="utf-8")
            ...
            artifact = (run / "paper_structure.json").read_text(encoding="utf-8")
