...
class WebAssetTests(unittest.TestCase):
    def test_static_fleet_pages_reference_only_local_assets_and_have_no_secret_markers(self) -> None:
        ...
        extensions_html = (web_dir / "extensions.html").read_text(encoding="utf-8")
        extensions_script = (web_dir / "extensions.js").read_text(encoding="utf-8")
        self.assertIn('id="extension-bundle-file"', extensions_html)
        self.assertIn("function extensionBasis()", extensions_script)
        ...
        for semantic_edge in ("retrieval_candidate", "source_provenance", "support", "contradict", "open_question", "condition_recorded", "citation_reference", "algorithmic_related", "crossref_reference", "identity_${mapping.status}"):
