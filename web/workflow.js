...
function renderReadingGuide(guide) {
  ...
  const contentLabel = { provider_advertised: "上游声明可读", confirmed: "读取已确认", failed_or_expired: "读取失败或已过期", metadata_only: "仅元数据" };
  target.replaceChildren(...items.map((item) => {
    ...
    const meta = document.createElement("p"); meta.className = "route-meta"; meta.textContent = `${wfText(item.source)} · ${item.publication_year || "年份未知"} · ${contentLabel[item.content_status] || "访问状态未知"}`;
